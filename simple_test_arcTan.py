...
for p in range(25,27):
    for q in range(15,17):
        ...
        for k in range(numIterations):
            ...
            for i in range(4):
                for j in range(4):
                    output = net.calculateOutput( [1,1,4], weights, [i,j], 0)
                    deltTemp = gd.gdBackprop(weights, output, net.derivArcTan, net.derivLeakyReLU, (.1/(p+q)), data[i][j])
...
df.to_csv('test_data\\simpletests_arcTan.csv', index = False)
